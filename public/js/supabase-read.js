...
function pointFromRow(row,regionName){
  ...
  return Object.assign(point,{
    ...
    regionId:internalRegionId(row.region_id),region:regionName,name:row.name,lat,lng,
...
function applySupabaseData(sb,regions,pointRows,imageRows){
  ...
  pointRows.forEach(row=>{const regionItem=regionBySupabaseId.get(String(row.region_id));if(!regionItem)return;const point=pointFromRow(row,regionItem.name);point.images=imagesByPoint.get(String(row.id))||[];groups.get(regionItem.id).push(point)});
